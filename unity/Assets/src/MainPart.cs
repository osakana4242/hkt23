...
namespace Osakana4242 {
	public sealed class MainPart : MonoBehaviour {
		...
		public GameObject hammberGo;
		public GameObject dollGo;
		...
		static StateMachine<MainPart>.StateFunc stateMain_g_ = (_evt) => {
			...
			if (Input.GetKeyDown(KeyCode.Z)) {
				{
					var hammerGo = self.hammberGo;
					var animator = hammerGo.GetComponentInChildren<Animator>();
					animator.CrossFadeInFixedTime("attack", 0.05f);
				}
				Observable.ReturnUnit().
					Delay(System.TimeSpan.FromSeconds(0.1f), Scheduler.MainThreadFixedUpdate).
					TakeUntilDestroy(self).
					Subscribe(_ => {
						var hammerGo = self.dollGo;
						var animator = hammerGo.GetComponentInChildren<Animator>();
						animator.CrossFadeInFixedTime("damage", 0.05f);
					});
			}
